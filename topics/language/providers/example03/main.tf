...
# Configure the `aws` provider and give it the alias `west`.data Read
# `access_key` and `secret_key` from the environment variables
...
# Create an `aws_instance` resource using the unaliased `aws` provider.
resource "aws_instance" "example_app_unaliased" {
  ami           = "ami-0c293f3f676ec4f90"
  instance_type = "t2.micro"
}

# Create an `aws_instance` resource using the `aws` provider with the alias
# `west`.
resource "aws_instance" "example_app_aliased" {
  provider = aws.west
